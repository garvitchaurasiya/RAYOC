...
const deploy = async()=>{  // The only reason to create the deploy method here is to use the async await syntax as we can't use await outside a method.
    ...
    const result = await new web3.eth.Contract(compiledCollegeRating.abi) // We are receiving JSON as the ABI in interface property we don't want to pass the json to create a contract that's why we are parsing(dividing) the JSON file into javascript object.
        .deploy({
